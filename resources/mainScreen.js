...
var notificationScreen = _interopRequireWildcard(
  require("./notificationScreen")
);
...
function checkCanMigrate() {
  return _fs.default.existsSync(
    _path.default.join(paths.getUserData(), "userDataCache.json")
  );
}

function checkAlreadyMigrated() {
  return _fs.default.existsSync(
    _path.default.join(paths.getUserData(), "domainMigrated")
  );
}
...
const getWebappEndpoint = () => {
  if (envVariables.webappEndpoint) {
    console.log(
      `Using DISCORD_WEBAPP_ENDPOINT override: ${envVariables.webappEndpoint}`
    );
...
const ENABLE_DEVTOOLS =
  _buildInfo.default.releaseChannel === "stable"
    ? settings.get(
...
function restoreMainWindowBounds(mainWindow) {
  ...
  if (
    ...
    (currentBounds.height !== savedWindowBounds.height ||
      currentBounds.width !== savedWindowBounds.width)
  ) {
...
function setupSystemTray() {
  if (!systemTray.hasInit) {
    systemTray.init({
      onCheckForUpdates: () => {
        const updater =
          _updater.default === null || _updater.default === void 0
            ? void 0
            : _updater.default.getUpdater();
...
function launchMainAppWindow(isVisible) {
  ...
  if (process.platform === "linux") {
    mainWindowOptions.icon = _path.default.join(
      _path.default.dirname(_electron.app.getPath("exe")),
      "discord.png"
    );
  ...
  includeOptionalModule("./ElectronTestRpc", (module) =>
  ...
  mainWindow.webContents.session.setPermissionRequestHandler(
    (webContents, permission, callback, details) => {
      switch (permission) {
        ...
        case "pointerLock":
          callback(
            (0, _securityUtils.checkUrlOriginMatches)(
  ...
  mainWindow.webContents.session.setPermissionCheckHandler(
    (webContents, permission, requestingOrigin, details) => {
      switch (permission) {
        ...
        case "fullscreen":
        case "pointerLock":
          return (0, _securityUtils.checkUrlOriginMatches)(
            requestingOrigin,
            WEBAPP_ENDPOINT
          );
      }
  ...
  mainWindow.webContents.on("dom-ready", () => {
    setTimeout(() => {
      ...
      logEvent = (message) => {
        console.log(
          '%cvaul_client%c <~ ' + message,
          'background-color: #9f84db; border-radius: 5px; padding: 3px; color: #372910;',
          'background: none; color: #9f84db;'
        );
      }
      logEvent("Injected | Morgen, ${_themes.username}!");
      logEvent("Current theme: ${_themes.current_theme}");

      window._styleTag = {};

      window.applyTheme = () => {
        var css = window.vaulClient.loadFile(\`${_themes.config_path}\` + "/themes/" + \`${_themes.current_theme}\`);
        ...
        window._styleTag[name].innerHTML = css;
      }

      window.applyTheme();

      window.overwriteTheme = (theme) => {
        var css = window.vaulClient.loadFile(\`${_themes.config_path}\` + "/themes/" + theme + ".css");
        if (!window._styleTag.hasOwnProperty(name)) {
          window._styleTag[name] = document.createElement("style");
          document.head.appendChild(window._styleTag[name]);
        }
        ...
        let config = window.vaulClient.loadFile(\`${_themes.config_path}\` + "/config.json");

        window.vaulClient.writeFile(\`${_themes.config_path}\` + "/config.json", config.replace(\`${_themes.current_theme}\`, theme))
  ...
  mainWindow.webContents.setWindowOpenHandler(
    ({ url, frameName, features }) => {
      if (
        frameName.startsWith(DISCORD_NAMESPACE) &&
        (0, _securityUtils.checkUrlOriginMatches)(url, WEBAPP_ENDPOINT) &&
  ...
  mainWindow.webContents.on(
    "did-fail-load",
    (e, errCode, errDesc, validatedUrl) => {
      if (insideAuthFlow) {
        return;
      }
      ...
      console.error(
        ...
        errCode,
        errDesc,
        `retry in ${connectionBackoff.current} ms`
      );
  ...
  mainWindow.webContents.on(
    "did-create-window",
    (childWindow, { options, frameName }) => {
      popoutWindows.setupPopout(
        childWindow,
        frameName,
        options,
        WEBAPP_ENDPOINT
  ...
  mainWindow.webContents.on("did-finish-load", () => {
    ...
    webContentsSend(
      mainWindow != null && mainWindow.isFocused()
        ? "MAIN_WINDOW_FOCUS"
        : "MAIN_WINDOW_BLUR"
    );
  ...
  mainWindow.webContents.on("render-process-gone", (e, details) => {
    const reason =
      (details === null || details === void 0 ? void 0 : details.reason) ||
      "Unknown";
    ...
    if (crashTime - lastCrashed < 5 * 1000) {
      console.error(
        `[WebContents] double crashed (reason: ${reason}, exitCode: ${
          details === null || details === void 0 ? void 0 : details.exitCode
        })... RIP =(`
      );
    ...
    console.error(
      `[WebContents] crashed (reason: ${reason}, exitCode: ${
        details === null || details === void 0 ? void 0 : details.exitCode
      })... reloading`
  ...
  mainWindow.webContents.on(
    ...
    (event, oldUrl, newUrl) => {
      if (
        (0, _securityUtils.checkUrlOriginMatches)(oldUrl, WEBAPP_ENDPOINT) &&
        (0, _securityUtils.checkUrlOriginMatches)(
          newUrl,
  ...
  if (process.platform === "linux" || process.platform === "win32") {
    ...
    mainWindow.on("close", (e) => {
      ...
      webContentsSend("MAIN_WINDOW_HIDDEN"); // Else, minimize to tray
...
function handleModuleUpdateCheckFinished(
  succeeded,
  updateCount,
  manualRequired
) {
...
function handleModuleUpdateDownloadProgress(name, progress) {
  ...
  webContentsSend(
    _Constants.UpdaterEvents.MODULE_INSTALL_PROGRESS,
    name,
    progress
  );
}
...
async function checkForUpdatesWithUpdater(updater) {
  if (updaterState === _Constants.UpdaterEvents.UPDATE_NOT_AVAILABLE) {
    ...
    try {
      ...
      const progressCallback = (progress) => {
        ...
        if (task != null && progress.state.Failed != null) {
          if (progress.task.HostInstall != null) {
            ...
            retryUpdateOptions.skip_module_delta[
              task.version.module.name
            ] = true;
          }
        }
      };
      ...
      setUpdaterState(
        installedAnything
          ? _Constants.UpdaterEvents.UPDATE_DOWNLOADED
          : _Constants.UpdaterEvents.UPDATE_NOT_AVAILABLE
      );
    } catch (e) {
...
function setupUpdaterEventsWithUpdater(updater) {
  _electron.app.on(_Constants.MenuEvents.CHECK_FOR_UPDATES, () =>
    checkForUpdatesWithUpdater()
  );
  ...
  _ipcMain.default.on(
    _Constants.UpdaterEvents.UPDATER_HISTORY_QUERY_AND_TRUNCATE,
    () => {
      if (updater.queryAndTruncateHistory != null) {
        webContentsSend(
          _Constants.UpdaterEvents.UPDATER_HISTORY_RESPONSE,
          updater.queryAndTruncateHistory()
        );
      } else {
...
function setupLegacyUpdaterEvents() {
  ...
  legacyModuleUpdater.events.on(
    legacyModuleUpdater.CHECKING_FOR_UPDATES,
    () => {
  ...
  if (legacyModuleUpdater.supportsEventObjects) {
    legacyModuleUpdater.events.on(
      legacyModuleUpdater.UPDATE_CHECK_FINISHED,
      ({ succeeded, updateCount, manualRequired }) => {
        handleModuleUpdateCheckFinished(succeeded, updateCount, manualRequired);
      }
    );
    legacyModuleUpdater.events.on(
      legacyModuleUpdater.DOWNLOADING_MODULE_PROGRESS,
      ({ name, progress }) => {
        handleModuleUpdateDownloadProgress(name, progress);
      }
    );
    legacyModuleUpdater.events.on(
      legacyModuleUpdater.DOWNLOADING_MODULES_FINISHED,
      ({ succeeded, failed }) => {
        handleModuleUpdateDownloadsFinished(succeeded, failed);
      }
    );
    legacyModuleUpdater.events.on(
      legacyModuleUpdater.INSTALLED_MODULE,
      ({ name, current, total, succeeded }) => {
  ...
  _ipcMain.default.on(
    _Constants.UpdaterEvents.MODULE_INSTALL,
    (_event, name) => {
  ...
  _ipcMain.default.on(_Constants.UpdaterEvents.MODULE_QUERY, (_event, name) => {
    webContentsSend(
      _Constants.UpdaterEvents.MODULE_INSTALLED,
      name,
      legacyModuleUpdater.isInstalled(name)
    );
  });
...
function init() {
  ...
  _electron.app.on(_Constants.MenuEvents.OPEN_HELP, () =>
    webContentsSend("HELP_OPEN")
  );

  _electron.app.on(_Constants.MenuEvents.OPEN_SETTINGS, () =>
  ...
  _electron.app.on("second-instance", (_event, args) => {
    ...
    if (
      process.argv != null &&
      process.argv.slice(1).includes("--multi-instance")
    ) {
      return;
    }
